...
class LM393:
    ...
    def calculate(self, elapse):
        if elapse != 0:  # to avoid DivisionByZero error
            ...
            print('RPM:{0:.0f} Speed:{1:.0f} Km/H Distance:{2:.2f}m Pulse:{3}'.format(self.rpm, km_per_hour, dist_meas,
                                                                                      self.pulse))
        pass
    ...
    def right_sensor_callback(self, channel):
        ...
        elapse = time.time() - self.start_timer
        self.start_timer = time.time()
        self.calculate(elapse)
